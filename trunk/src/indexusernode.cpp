...
const int IndexUserNode::BYTESREDONDEO = 4;  // (_hnoIzquierdo + _hnoDerecho) (nodobmashoja) - _hijoIzquierdo (nodobmasindice)

IndexUserNode::IndexUserNode(int Numero, int Nivel, int Padre, int HijoIzquierdo, char* key, int ref)
                               : UserNode(Numero, Nivel, Padre)
{   
...
void IndexUserNode::read(char* buffer)
{
...
void IndexUserNode::write(char* buffer)
{
    char* nextByte = buffer;
...
int IndexUserNode::search(const char* key)
{
...
UserNode* IndexUserNode::split(int Numero, char* arreglo,int bytesArreglo,int clavesArreglo,
							char** claveAlPadre)
{
	IndexUserNode* ret;
    int auxRef;
	...
	if(( clavesArreglo == 1)&&(_espacioLibre < TAMARREGLO /2)){

		int clavesLeidas = 0;

		while(offsetArreglo < TAMARREGLO /2){
			// tamanio de la clave
		...
		for(int i = clavesLeidas; i < _nclaves;i++){
			...
			if(!clavesCopiadas)
			{
				*claveAlPadre = new char[(longClave + 1) * sizeof(char)];
				...
				ret = new IndexUserNode(Numero,_nivel,_padre,auxRef);
...
void IndexUserNode::promoteRoot(UserNode** nodo1,UserNode** nodo2,int id1,int id2,
...
{   
    ...
    *nodo1 = new IndexUserNode(id1,_nivel,_id,_hijoIzquierdo);
	...
	while((*nodo1)->getOffset() < TAMARREGLO / 2)
	{
    ...
    *nodo2 = new IndexUserNode(id2,_nivel,_id,auxRef);
