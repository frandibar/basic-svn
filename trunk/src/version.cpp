...
Version::Version(int NroVersion, int Original, long int Fecha,
				 char* User,long int Offset,char Tipo){
...
void Version::write(char* buffer)
{
	...
	memcpy(buffer,&_fecha,sizeof(long int));
...
void Version::read(char* buffer)
{
	...
	memcpy(&_fecha,buffer,sizeof(long int));
	buffer += sizeof(long int);
