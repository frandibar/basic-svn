...
LeafUserNode::LeafUserNode(int Numero, int Padre, int HnoIzquierdo, int HnoDerecho)
    : UserNode(Numero, 0, Padre)
{
    _hnoIzquierdo = HnoIzquierdo;
    _hnoDerecho   = HnoDerecho;
}

void LeafUserNode::read(char* buffer)
{
...
void LeafUserNode::write(char* buffer)
{
    char* nextByte = buffer;
...
int LeafUserNode::search(const char* key)
{
    char* auxKey;
	char* auxFileName;
	int longClave;
...
UserNode* LeafUserNode::split(int Numero, 
							  char* arreglo,int bytesArreglo,int clavesArreglo,
							  char** claveAlPadre)
{
	int longClave = 0;
	int offsetArreglo = 0;
    LeafUserNode* ret = new LeafUserNode(Numero, _padre, _id, _hnoDerecho);
...
void LeafUserNode::promoteRoot(UserNode** nodo1,UserNode** nodo2, int id1, int id2,
							   int clavesArreglo,char** arreglo, int bytesArreglo,
							   char** claveARaiz)
{
	// creo los dos nodos que van a ser los hijos de la raiz
   *nodo1 = new LeafUserNode(id1, _id, -1, id2);

	*nodo2 = new LeafUserNode(id2,_id,id1,-1);
